...
# TODO: Task c: draw another rectangle (parameters: x1=400, width=30, y1=50, height=120)
li $a0,400   # x1 for second rectangle
li $a1,30    # width for second rectangle
li $a2,50    # y1 for second rectangle
li $a3,120   # height for second rectangle
jal rectangle
...
# Originally, the code was set to blue with:
# li $t0,0xFF0000FF # color: blue
#
# We need magenta (#FF00FF), which can be loaded as:
li $t0,0x00FF00FF  # color: magenta (ARGB: 00 FF 00 FF)
...
addu $a3,$a3,$a0 # last row ending pixel address (base + y2_scaled + x2_scaled)
...
rectangleXloop:
    sw $t0,($t3)           # paint this pixel with current color in $t0
    ...
    # Task d: Add gradient effect by adjusting the color each row
    addi $t0,$t0,105536      # increment the red channel significantly to create a visible stripe
                            # (0x00010000 affects the red channel in 0x00RRGGBB format)
